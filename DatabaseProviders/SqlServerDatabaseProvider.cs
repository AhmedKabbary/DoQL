        ...
        public string Export(Database db)
        {
            ...
            foreach (Table t in db.Tables)
            {
                ...
                foreach (Column C in t.Columns)
                {
                    ...
                    if (C.AutoIncrement == true) { constraints.Append("IDENTITY(1,1) "); }
                ...
                if (t.ForiegnReferences.Count > 0)
                {
                    foreach (ForiegnReference f in t.ForiegnReferences)
                    {
                        ...
                        string onUpdate = "";
                        string onDelete = "";
                        if (f.OnUpdateAction != Action.NoAction && f.OnUpdateAction != Action.Restrict)
                        {
                            onUpdate = $"ON UPDATE {ActionFactory.GetActionString(f.OnUpdateAction)} ";
                        }
                        if (f.OnDeleteAction != Action.NoAction && f.OnUpdateAction != Action.Restrict)
                        {
